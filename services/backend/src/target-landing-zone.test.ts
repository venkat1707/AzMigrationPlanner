...
test('parses the subscription id and resource group name from a resource group ID', () => {
  const derived = deriveResourceGroup({ resourceGroupId: `/subscriptions/${subscription}/resourceGroups/vt-arc-rg` })
...
test('trims surrounding whitespace from the resource group ID', () => {
  const derived = deriveResourceGroup({ resourceGroupId: `  /subscriptions/${subscription}/resourceGroups/vt-migplanner-rg  ` })
...
test('rejects a non-GUID subscription id', () => {
  assert.throws(() => deriveResourceGroup({ resourceGroupId: '/subscriptions/not-a-guid/resourceGroups/vt-arc-rg' }), /subscription ID/i)
})

test('rejects an ID that includes a provider or child resource', () => {
  assert.throws(() => deriveResourceGroup({ resourceGroupId: `/subscriptions/${subscription}/resourceGroups/vt-arc-rg/providers/Microsoft.Network/networkSecurityGroups/app-nsg` }), /resource group only/i)
})

test('rejects an ID that is missing the resource group segment', () => {
  assert.throws(() => deriveResourceGroup({ resourceGroupId: `/subscriptions/${subscription}` }), /resourceGroups/i)
})

test('requires a resource group ID', () => {
  assert.throws(() => deriveResourceGroup({ resourceGroupId: '   ' }), /required/i)
})
