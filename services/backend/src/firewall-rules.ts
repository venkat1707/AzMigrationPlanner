...
function renderLandingZoneReadme(landingZone: LandingZoneContext): string {
  ...
  const rows = landingZone.placements.map((placement) => `| ${placement.servers.join(', ')} | ${placement.subscriptionName || placement.subscriptionId || '\u2014'} | ${placement.resourceGroupName ?? '\u2014'} | ${placement.virtualNetwork ?? '\u2014'} | ${placement.subnet ?? '\u2014'}${placement.subnetIpSegment ? ` (${placement.subnetIpSegment})` : ''} | ${placement.networkSecurityGroup || '\u2014'} |`).join('\n')
...
| Servers | Subscription | Resource group | Virtual network | Subnet | NSG |
|---|---|---|---|---|---|
${rows}${unmappedNote}
`
}
