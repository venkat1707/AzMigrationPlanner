...
// The values below (landing zone placements, service ports/protocols, sprint labels) originate from
// imported spreadsheets/agent-parsed configs and are never validated against a strict charset, so they
// must be escaped before being embedded in generated Terraform/Bicep/shell text — otherwise a crafted
// value (e.g. a resource group name containing a quote) could break out of its string literal and inject
// arbitrary generated code the user later applies/executes.
function tfString(value: string): string {
  return value
    .replace(/\\/g, '\\\\')
    .replace(/"/g, '\\"')
    .replace(/\$\{/g, () => '$${')
...
// For values embedded in a double-quoted bash command inside generated README instructions.
...
// For values embedded in a `#`/`//` single-line comment, where a raw newline would let the rest of the
// value escape the comment and be interpreted as code.
...
function tfStringList(values: string[]): string {
  return `[${values.map((value) => `"${tfString(value)}"`).join(', ')}]`
}

function bicepStringList(values: string[]): string {
  return `[${values.map((value) => `'${bicepString(value)}'`).join(', ')}]`
}
...
function terraformHeader(ruleSet: FirewallRuleSet): string {
  return [
    ...
    `# Scope: ${singleLine(ruleSet.scopeLabel)}`,
...
variable "sprint_address_space" {
  ...
  default     = "${tfString(addressSpaceDefault)}"
}
`)
...
${landingZoneReadme}`)
  ...
  default     = "${tfString(primaryPlacement.resourceGroupName)}"` : ''}
}
...
variable "location" {
  ...
  default     = "${tfString(primaryPlacement.location)}"` : ''}
}
...
variable "nsg_name" {
  ...
  default     = "${tfString(primaryPlacement.networkSecurityGroup)}"` : ''}
}
...
`)
  ...
  name                 = "${tfString(placement.subnet ?? '')}"
  virtual_network_name = "${tfString(placement.virtualNetwork ?? '')}"
  resource_group_name  = "${tfString(placement.resourceGroupName ?? '')}"
}
...
az deployment group create \\
  --resource-group "${shellDoubleQuoted(resourceGroupExample)}" \\
...
${landingZoneReadme}`)
  ...
  const nsgRuleLiterals = projected.map((rule) => `  {
    ...
    priority: ${rule.priority}
    direction: '${rule.direction}'
    protocol: '${bicepString(rule.protocol)}'
    port: '${bicepString(rule.portRange)}'
    sourcePrefixes: ${bicepStringList(rule.sourceAddresses)}
    destinationPrefixes: ${bicepStringList(rule.destinationAddresses)}
    description: '${bicepString(rule.description)}'
  }`).join('\n')

  zip.file('nsg.bicep', `// Generated by Cloud Accelerate Factory
// Scope: ${singleLine(ruleSet.scopeLabel)}
param location string${primaryPlacement?.location ? ` = '${bicepString(primaryPlacement.location)}'` : ''}
param nsgName string${primaryPlacement?.networkSecurityGroup ? ` = '${bicepString(primaryPlacement.networkSecurityGroup)}'` : ''}
param sprintAddressPrefixes array = ${addressPrefixesLiteral}
...
param location string = ${primaryPlacement?.location ? `'${bicepString(primaryPlacement.location)}'` : 'resourceGroup().location'}
param nsgName string${primaryPlacement?.networkSecurityGroup ? ` = '${bicepString(primaryPlacement.networkSecurityGroup)}'` : ''}
param sprintAddressPrefixes array = ${addressPrefixesLiteral}
...
`)
  ...
  const subnetAssociationNote = subnetPlacements.length > 0
    ? `\n## Associate The NSG With The Sprint's Subnets\n\nBicep cannot safely patch an existing subnet without restating all of its properties, so associate the NSG after deploying it:\n\n\`\`\`bash\n${subnetPlacements.map((placement) => `az network vnet subnet update --resource-group "${shellDoubleQuoted(placement.resourceGroupName ?? '')}" --vnet-name "${shellDoubleQuoted(placement.virtualNetwork ?? '')}" --name "${shellDoubleQuoted(placement.subnet ?? '')}" --network-security-group <nsg-id-or-name>`).join('\n')}\n\`\`\`\n`
    : ''
