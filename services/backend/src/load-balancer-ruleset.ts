...
export const loadBalancerRulesetAgentInstructions = [
  ...
  'A rule schema entry supports exactly ONE conditionGroup guarding ONE ordered list of actions. When the source has a multi-branch construct (an iRule/policy with if / elseif / else, or a NetScaler policy with a fallback action) where DIFFERENT branches lead to DIFFERENT actions or targets, do NOT invent an unlisted field (such as a per-action condition) to encode the branching — instead emit ONE separate rule row per branch, each with its own conditionGroup (negating the prior branches\' conditions is not required, just express that branch\'s own condition), the SAME virtualServerExternalId, and sequential priority values in the branch\'s evaluation order (lower priority number evaluates first). Suffix each branch\'s externalId to keep them unique and traceable to the source (e.g. "-branch1", "-branch2", "-else"); an unconditional else/default branch gets conditionGroup: null.',
