...
async function callRulesetAgent(agent: AgentRow, vendor: string | null, format: string, fileName: string, rawContent: string): Promise<Record<string, unknown>> {
  ...
  if (responseStatus === 'incomplete') {
    ...
    console.error(`Load balancer ruleset agent response was incomplete (reason: ${reason}) from ${requestUrl}.`)
    throw new LoadBalancerRulesetError(`The load balancer ruleset agent's response was cut off before finishing (${reason === 'max_output_tokens' ? 'it hit the model\u2019s max output token limit' : reason}). This export may be too large for the agent's configured output length; try increasing the agent's max output tokens or splitting the export.`, 502)
  }
  ...
  if (!contract) {
    ...
    console.error(`Load balancer ruleset agent response was not valid JSON from ${requestUrl}: ${assistantText.slice(0, 500)}${assistantText.length > 500 ? '\u2026' : ''} <<<END>>> ${assistantText.length > 500 ? assistantText.slice(-500) : ''}`)
    throw new LoadBalancerRulesetError(`The load balancer ruleset agent returned a response that was not valid JSON. Response started with: ${snippet}`, 502)
  }
