...
import { AlertCircle, CalendarDays, CheckCircle2, FileSpreadsheet, Presentation, RefreshCw, Save } from 'lucide-react'
...
type ServerTimeline = { serverName: string; sprintSequence: number | null; targetedStartDate: string | null; targetedEndDate: string | null }
type ScheduleResponse = { waves: ScheduleWave[]; serverTimeline: ServerTimeline[]; savedAt: string | null }
type EditableDates = Record<number, { targetedStartDate: string; targetedEndDate: string }>
...
export default function SprintSchedule() {
  ...
  const updateDate = (sequence: number, field: 'targetedStartDate' | 'targetedEndDate', value: string) => {
  ...
  const save = async () => {
    ...
    try {
      ...
      const payload = await response.json() as { schedules?: Array<{ sequence: number; targetedStartDate: string; targetedEndDate: string }>; error?: string }
  ...
  return <div className="page sprint-schedule-page">
    ...
    <section className="schedule-table-section" aria-labelledby="schedule-table-heading">
      ...
      {waves.length === 0 ? <div className="schedule-empty"><CalendarDays size={25} /><strong>No saved migration plan</strong><span>Generate and save a wave plan before scheduling sprints.</span></div> : <>
        <div className="schedule-table-wrap"><table className="schedule-table"><thead><tr><th>Environment</th><th>Wave</th><th>Sprint</th><th>Servers</th><th>Applications</th><th>Targeted start</th><th>Targeted end</th></tr></thead><tbody>{visibleSprints.map((sprint) => <tr key={sprint.sequence}><td>{sprint.environment}</td><td>Wave {sprint.wave}</td><td><strong>{sprint.name}</strong><small>Sequence {sprint.sequence}</small></td><td>{sprint.serverCount.toLocaleString()}</td><td><span className="schedule-app-summary" title={sprint.applications.join(', ')}><strong>{sprint.applications.length} apps</strong><small>{sprint.applications.slice(0, 3).join(', ')}{sprint.applications.length > 3 ? ` +${sprint.applications.length - 3}` : ''}</small></span></td><td><input type="date" aria-label={`Targeted start for ${sprint.name}`} value={dates[sprint.sequence]?.targetedStartDate ?? ''} onChange={(event) => updateDate(sprint.sequence, 'targetedStartDate', event.target.value)} /></td><td><input type="date" min={dates[sprint.sequence]?.targetedStartDate || undefined} aria-label={`Targeted end for ${sprint.name}`} value={dates[sprint.sequence]?.targetedEndDate ?? ''} onChange={(event) => updateDate(sprint.sequence, 'targetedEndDate', event.target.value)} placeholder={dates[sprint.sequence]?.targetedStartDate ? addDays(dates[sprint.sequence]!.targetedStartDate, 21) : undefined} /></td></tr>)}</tbody></table></div>
    ...
    <GanttChart sprints={scheduled} exporting={exporting} onExport={exportSchedule} />
  </div>
}

function GanttChart({ sprints, exporting, onExport }: { sprints: Array<ScheduleSprint & { wave: number; environment: string; start: string; end: string }>; exporting: 'xlsx' | 'pptx' | null; onExport: (format: 'xlsx' | 'pptx') => Promise<void> }) {
  ...
  return <section className="schedule-gantt-section" aria-labelledby="schedule-gantt-heading">
    ...
    <div className="gantt-scroll"><div className="gantt-chart">
      <div className="gantt-axis-label">Wave / Sprint</div><div className="gantt-axis">{ticks.map((tick) => <span key={tick} style={{ left: `${((tick - minimum) / Math.max(millisecondsPerDay, maximum - minimum)) * 100}%` }}>{dateFormatter.format(tick)}</span>)}</div>
      {grouped.flatMap((wave) => sprints.filter((sprint) => sprint.wave === wave).map((sprint, index) => {
        ...
        return [
          ...
          <div className="gantt-row" key={sprint.sequence}><span className="gantt-sprint-name" title={sprint.applications.join(', ')}><strong>{sprint.name}</strong><small>{sprint.applications.length} apps · {sprint.applications.slice(0, 3).join(', ')}{sprint.applications.length > 3 ? ` +${sprint.applications.length - 3}` : ''}</small></span><div className="gantt-grid">{ticks.map((tick) => <i key={tick} style={{ left: `${((tick - minimum) / Math.max(millisecondsPerDay, maximum - minimum)) * 100}%` }} />)}<div className={`gantt-bar gantt-wave-${wave % 4}`} style={{ left: `${left}%`, width: `${width}%` }} title={`${sprint.name}: ${sprint.start} to ${sprint.end}\nApplications: ${sprint.applications.join(', ')}`}><strong>{sprint.name}</strong><small>{sprint.start} – {sprint.end}</small></div></div></div>,
        ]
      }))}
    </div></div>
  </section>
