...
export default function VisualizeSprints() {
  ...
  const [showSprintRings, setShowSprintRings] = useState(true)
  ...
  const crossSprintIntensity = useMemo(() => {
    const weights = new Map<string, number>()
    if (!view) return { weights, maximum: 0 }
    const nodes = new Map(view.nodes.map((node) => [node.id, node]))
    for (const edge of view.edges) {
      const source = nodes.get(edge.from)
      const target = nodes.get(edge.to)
      if (!source || !target || source.sprint === target.sprint) continue
      weights.set(source.id, (weights.get(source.id) ?? 0) + edge.weight)
      weights.set(target.id, (weights.get(target.id) ?? 0) + edge.weight)
    }
    return { weights, maximum: Math.max(0, ...weights.values()) }
  }, [view])
  ...
  return <div className="page sprint-visualization-page">
    <section className="sprint-visualization-toolbar"><div><span className="eyebrow">Dependency proximity</span><strong>Visualize sprints</strong><small>{selectedSprint === null ? showSprintRings ? 'Click a dotted sprint boundary to focus its applications or servers and internal relationships.' : 'Sprint rings are hidden. Brighter dots have more dependencies crossing sprint boundaries.' : `Focus mode: Sprint ${selectedSprint} and its ${view.nodes.length} ${mode}.`}</small></div><div className="visualization-controls"><label>Environment<select value={environment} onChange={(event) => changeEnvironment(event.target.value)}><option>All</option>{environments.map((item) => <option key={item}>{item}</option>)}</select></label><label>Sprint<select value={selectedSprint ?? ''} onChange={(event) => changeSprint(event.target.value)}><option value="">All sprints</option>{sprintOptions.map((sprint) => <option key={sprint.sequence} value={sprint.sequence}>Sprint {sprint.sequence}</option>)}</select></label><div className="visualization-mode" role="group" aria-label="Visualize by"><button type="button" className={mode === 'applications' ? 'active' : ''} onClick={() => changeMode('applications')}><AppWindow size={15} /> Applications</button><button type="button" className={mode === 'servers' ? 'active' : ''} onClick={() => changeMode('servers')}><Server size={15} /> Servers</button></div><div className="visualization-mode" role="group" aria-label="Sprint rings"><button type="button" className={showSprintRings ? 'active' : ''} aria-pressed={showSprintRings} onClick={() => setShowSprintRings(true)}><CircleDotDashed size={15} /> Rings</button><button type="button" className={!showSprintRings ? 'active' : ''} aria-pressed={!showSprintRings} onClick={() => setShowSprintRings(false)}>No rings</button></div></div></section>
    <section className="sprint-visualization-canvas"><div className="visualization-legend">{showSprintRings && <span><i className="ring" /> {selectedSprint === null ? 'Click sprint boundary to focus' : `Focus mode: Sprint ${selectedSprint}`}</span>}<span><i className="edge" /> KNN dependency link</span><span><i className="intensity" /> Brighter dot = more cross-sprint dependencies</span><span><Users size={14} /> {view.sprints.length} sprint{view.sprints.length === 1 ? '' : 's'}</span><span><Network size={14} /> {view.nodes.length} {mode}</span><div className="visualization-zoom"><button type="button" onClick={() => setZoom((value) => Math.max(.7, Number((value - .15).toFixed(2))))} disabled={zoom <= .7} aria-label="Zoom out"><ZoomOut size={15} /></button><span>{Math.round(zoom * 100)}%</span><button type="button" onClick={() => setZoom((value) => Math.min(3, Number((value + .15).toFixed(2))))} disabled={zoom >= 3} aria-label="Zoom in"><ZoomIn size={15} /></button></div>{selectedSprint !== null && <button type="button" className="visualization-clear-focus" onClick={() => { setSelectedSprint(null); setHoveredNode(null); resetViewport() }}><X size={14} /> All sprints</button>}</div><svg className={panning ? 'panning' : ''} viewBox={`0 0 ${width} ${height}`} preserveAspectRatio="xMidYMid meet" role="img" aria-label={`KNN dependency proximity map for ${mode}`} onPointerDown={beginPan} onPointerMove={movePan} onPointerUp={endPan} onPointerCancel={endPan}>
      ...
      <g transform={`translate(${width / 2} ${height / 2}) scale(${zoom}) translate(${-width / 2} ${-height / 2}) translate(${pan.x} ${pan.y})`}>
        ...
        {view.nodes.map((node) => { const ring = rings.find((item) => item.sprint === node.sprint)!; const dotRadius = focused ? (mode === 'applications' ? 9 : 7) : (mode === 'applications' ? 6 : 5); const crossSprintWeight = crossSprintIntensity.weights.get(node.id) ?? 0; const intensity = crossSprintIntensity.maximum > 0 ? Math.log1p(crossSprintWeight) / Math.log1p(crossSprintIntensity.maximum) : 0; return <g key={node.id} className={`sprint-map-node${intensity > .66 ? ' high-cross-sprint' : intensity > .25 ? ' medium-cross-sprint' : ''}`} data-cross-sprint-weight={crossSprintWeight} data-cross-sprint-intensity={intensity.toFixed(3)} transform={`translate(${node.x} ${node.y})`} onMouseEnter={() => setHoveredNode(node)} onMouseLeave={() => setHoveredNode(null)}>{intensity > 0 && <circle className="cross-sprint-halo" r={dotRadius + 5 + intensity * 5} opacity={.12 + intensity * .32} />}<circle className="sprint-node-dot" r={dotRadius + intensity * 2.5} fill={ring.color} style={{ filter: `brightness(${1 + intensity * .9}) saturate(${1 + intensity * .55}) drop-shadow(0 0 ${Math.round(intensity * 7)}px rgb(234 169 37 / ${(.25 + intensity * .65).toFixed(2)}))` }} />{focused && <text y={mode === 'applications' ? 25 : 23} textAnchor="middle">{node.label.length > 20 ? `${node.label.slice(0, 19)}…` : node.label}</text>}<title>{node.label} · Sprint {node.sprint} · {node.serverNames.length} server{node.serverNames.length === 1 ? '' : 's'} · ${formatNumber.format(crossSprintWeight)} cross-sprint dependencies</title></g> })}
